...
class FacilitiesRepository(BaseRepository):
    model = FacilitiesOrm
    mapper: FacilityDataMapper = FacilityDataMapper


class RoomsFacilitiesRepository(BaseRepository):
    model = RoomsFacilitiesOrm
    mapper: RoomFacilityDataMapper = RoomFacilityDataMapper
    ...
    async def set_room_facilities(
        ...
        if ids_to_delete:
            delete_m2m_fasilities_stmt = delete(self.model).filter(
                self.model.room_id == room_id, self.model.facility_id.in_(ids_to_delete)
            )
            await self.session.execute(delete_m2m_fasilities_stmt)

        if ids_to_insert:
            insert_m2m_fasilities_stmt = insert(self.model).values(
                [{"room_id": room_id, "facility_id": f_id} for f_id in ids_to_insert]
            )
            await self.session.execute(insert_m2m_fasilities_stmt)
